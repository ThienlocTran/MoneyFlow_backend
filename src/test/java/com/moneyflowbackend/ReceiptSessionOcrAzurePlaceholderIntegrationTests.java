...
@SpringBootTest(properties = "MONEYFLOW_RECEIPT_OCR_PROVIDER=azure_document_intelligence")
@AutoConfigureMockMvc
@ActiveProfiles("test")
@Transactional
class ReceiptSessionOcrAzurePlaceholderIntegrationTests {
    ...
    @Test
    void azurePlaceholderReturnsNotImplementedWithoutTransaction() throws Exception {
        ...
        mockMvc.perform(post("/api/workspaces/{workspaceId}/receipt-sessions/{sessionId}/ocr", owner.workspace().getId(), sessionId)
                ...
                .andExpect(jsonPath("$.data.warnings[0].code").value("OCR_PROVIDER_NOT_IMPLEMENTED"));
